...
app.get("/api/ytsearch", async (req, res) => {
    ...
    const data = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&order=relevance&q=${gameTitleSearch}&type=video&videoDefinition=high&maxResults=15&topicId=/m/0bzvm2&key=${apiKey}`)
        .then((response) => {
        if (response.ok)
